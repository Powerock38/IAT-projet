...
class SpaceInvaders():
    ...
    def get_state(self):
        ...
        """
        # return [self.get_player_X(), self.get_invaders_X()[0]]

        # downscale self.full_image() to 64x64
        # start time
        #t = time.time()

        # get x position of the player from the full image
        # get the line of pixels corresponding to the player
        ...
        player_position_x = np.where(player_line > 0)[0]

        # if the player is not on the line, return the last state
        if len(player_position_x) == 0:
            player_position_x = 0
        # else, return the middle of the player
        else:
            player_position_x = int(np.mean(player_position_x))
