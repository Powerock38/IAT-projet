...
class DQNAgent:
    ...
    def __init__(self, qnetwork: nn.Module, eps_profile: EpsilonProfile, gamma: float, alpha: float, replay_memory_size: int = 1000, batch_size: int = 32, target_update_freq: int = 100, tau: float = 1., final_exploration_episode : int = 500, na : int = 4):
        """
    ...
    def init_replay_memory(self, env: SpaceInvaders):
        ...
        self.Ds = np.zeros([self.replay_memory_size, 1, env.nx, env.ny], dtype=np.float32)
        ...
        self.Dsn = np.zeros([self.replay_memory_size, 1, env.nx, env.ny], dtype=np.float32)
    ...
    def updateQ(self, state, action, reward, next_state, terminal):
        ...
        self.Ds[self.d] = state
        self.Dr[self.d] = reward
        self.Dsn[self.d] = next_state
        self.Dt[self.d] = terminal
